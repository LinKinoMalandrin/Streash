...
public class Mapwhere extends AbstractFunction {
	...
	/**
	 * 
	 * @return a new map whose elements are the elements of the stream that are
	...
	public Value evaluate() {
		...
		if (args[0] instanceof StreamVar && args[1] instanceof LambdaVar && args[2] instanceof CharChain
				&& args[3] instanceof Primitive && args[4] instanceof LambdaVar) {
			return Stream.getVar((StreamVar) args[0], (LambdaVar) args[1], (CharChain) args[2], (Value) args[3],
					(LambdaVar) args[4]);
		}
	...
	public static abstract class Stream extends AbstractStreamVar {
		...
		public static StreamVar getVar(StreamVar s, LambdaVar filter, CharChain operator, Value operand,
				LambdaVar map) {
			...
			if (t.hasNext()) {
				...
				if (!m.getType().equals(v.getType()))
					throw new IllegalStateException(
							"Cannot have a mapping function which signature does not match with original stream's elements");
				if (!operand.getType().equals(filter.produce(v).getType()))
					throw new IllegalStateException(
							"Cannot fave a filtering function which produces elements of an other type than the given operand");
				if (v instanceof Number)
					...
					throw new IllegalArgumentException("Return type of mapping function is not a primitive");
			}
